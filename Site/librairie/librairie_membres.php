...
function tableau_membre_non_valide() {
    ...
    if ($query->rowCount() == 0) {
        ...
        while (($data = $query->fetch(PDO::FETCH_ASSOC)) !== false) {
            ...
            echo '<td><a href="suppression-validation-membres.php?id_membre_valide=' . $data["id_membre"] . '"><span class="glyphicon glyphicon-ok"></span></a></td>';
            echo '</tr>';
        }
    }
}
...
function tableau_membre_valide() {
    ...
    while (($data = $query->fetch(PDO::FETCH_ASSOC)) !== false) {
        ...
        echo '<td>' . $data['prenom'] . '</td>';
        echo '<td>' . date_format(date_create($data['date_naissance']), "l d F") . '</td>';
        echo '<td><a href="creer-modifier-membres.php?id_membre_modification=' . $data["id_membre"] . '"><span class="glyphicon glyphicon-wrench"></span></a>' . " " . '<a href="suppression-validation-membres.php?id_membre_suppression=' . $data["id_membre"] . '"><span class="glyphicon glyphicon-trash"></span></a></td>';
        echo '</tr>';
    }
}
